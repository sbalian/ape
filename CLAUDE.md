...
3. `call_llm()` wraps `pydantic_ai.Agent`, which is the provider abstraction. It sets
   ...
   **`APE_API_KEY`** env var — Ape's own variable, deliberately *not* a provider's
   ...
   call. `call_llm()` also sets `pydantic_ai.BANNER_ENABLED = False` before building
   the agent: recent Pydantic AI versions print a first-run banner (logo plus an
   "observability: off" block) to **stderr** on the first agent run in a terminal,
   which would sit next to Ape's one-line command output. That attribute is Pydantic
   AI's documented switch for it. This is the **one** `ty: ignore` in the codebase
   (`invalid-assignment`), and it is deliberate: Pydantic AI declares the flag as a
   bare `BANNER_ENABLED = True` with no annotation, so ty infers `Literal[True]` and
   rejects assigning `False` — the very thing the attribute exists for. No
   `pydantic-ai-slim` floor bump is needed: the assignment is a harmless no-op on
   versions that predate the banner. Note the banner is invisible to the test suite
   regardless — Pydantic AI suppresses it whenever `PYTEST_VERSION`, `CI` or
   `PYDANTIC_AI_NO_BANNER` is in the environment — so a regression here would only
   show up when a user runs `ape` in a terminal.
...
`test_call_llm_disables_the_pydantic_ai_banner` runs the real `call_llm` (with
`TestModel` swapped in for `infer_model`) and asserts the banner flag ends up `False`.
It sets the flag back to `True` via `monkeypatch.setattr(..., raising=False)` first, so
the check is meaningful and monkeypatch restores the global afterwards.
