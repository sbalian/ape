...
def test_app_for_suggestion(mockenv, monkeypatch):
    monkeypatch.setattr("ape.call_llm", lambda *args, **kwargs: "ls")
    result = runner.invoke(ape.app, ["list all the files"])
...
def test_app_for_try_again_if_api_returns_none(mockenv, monkeypatch):
    monkeypatch.setattr("ape.call_llm", lambda *args, **kwargs: None)
    result = runner.invoke(ape.app, ["what is the capital of England?"])
...
def test_app_with_api_error(mockenv, monkeypatch):
    ...
    monkeypatch.setattr("ape.call_llm", mockreturn)
    result = runner.invoke(ape.app, ["list all the files"])
...
def test_app_for_suggestion_with_execute(mockenv, monkeypatch):
    monkeypatch.setattr("ape.call_llm", lambda *args, **kwargs: "ls")
    result = runner.invoke(ape.app, ["list all the files", "--execute"])
